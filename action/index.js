...
function getInputs() {
    const result = {};
    let filePath = core.getInput(constants_1.Inputs.Path, { required: true });
    if (filePath == null) {
        throw new Error('You must provide either \'path\' in your configuration.');
    }
    else {
        if (filePath.startsWith('~/') || filePath.startsWith('~\\'))
            filePath = (0, path_1.join)((0, os_1.homedir)(), filePath.slice(1));
        else if (filePath === '~')
            filePath = (0, os_1.homedir)();
        result.path = (0, path_1.resolve)(process.cwd(), filePath);
    }
    result.contents = core.getInput(constants_1.Inputs.Contents, { required: true });
    result.writeMode = core.getInput(constants_1.Inputs.WriteMode, { required: true });
    ...
    result.encoding = core.getInput(constants_1.Inputs.Encoding, { required: false });
...
function setOutputs(response) {
    core.setOutput(constants_1.Outputs.Size, response.size);
...
const fs_1 = __nccwpck_require__(147);
const path_1 = __nccwpck_require__(17);
const io_helper_1 = __nccwpck_require__(262);
(function run() {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const inputs = (0, io_helper_1.getInputs)();
            if (inputs.writeMode !== 'preserve' || !(0, fs_1.existsSync)(inputs.path)) {
                core.info(`Writing ${inputs.path} file.`);
                const targetDir = (0, path_1.dirname)(inputs.path);
                yield (0, io_1.mkdirP)(targetDir);
                if (inputs.writeMode === 'overwrite') {
                    (0, fs_1.writeFileSync)(inputs.path, inputs.contents, inputs.options);
                }
                else {
                    (0, fs_1.appendFileSync)(inputs.path, inputs.contents, inputs.options);
                }
            ...
            const result = (0, fs_1.statSync)(inputs.path);
            (0, io_helper_1.setOutputs)(result);
        }
        catch (err) {
            core.setFailed((_a = err === null || err === void 0 ? void 0 : err.message) !== null && _a !== void 0 ? _a : err);
        }
    });
})();
